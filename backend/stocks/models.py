...
class ScoreSnapshot(models.Model):
    ...
    is_investment_ineligible = models.BooleanField(default=False, db_index=True)
    red_flag_reasons = models.JSONField(default=list, blank=True)
    action_signal = models.CharField(max_length=32, default="REVIEW", db_index=True)
    action_label = models.CharField(max_length=80, default="평가 보류")
    financial_data_status = models.CharField(max_length=24, default="partial")
    market_validation_score = models.FloatField(null=True, blank=True, db_index=True)
    valuation_adjustment = models.FloatField(default=0)
